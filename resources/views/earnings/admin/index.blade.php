                    ...
                    <table class="w-full table whitespace-nowrap">
                        ...
                        @forelse($earnings as $earning)
                            <tr>
                                ...
                                <td class="py-3 pl-5">
                                    @if($earning->trip->status === 'Active')
                                        <x-status :content="$earning->trip->status" :type="'info'" />
                                    @elseif($earning->trip->status === 'Confirmed')
                                        <x-status :content="$earning->trip->status" :type="'success'" />
                                    @elseif($earning->trip->status === 'In Progress')
                                        <x-status :content="$earning->trip->status" :type="'warning'" />
                                    @elseif($earning->trip->status === 'Completed')
                                        <x-status :content="$earning->trip->status" :type="'success'" />
                                    @elseif($earning->trip->status === 'Cancelled')
                                        <x-status :content="$earning->trip->status" :type="'error'" />
                                    @endif
                                </td>
                                <td class="py-3 pl-5">
