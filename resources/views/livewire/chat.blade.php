            ...
            <div class="flex flex-col sm:flex-row gap-5 justify-between">
                ...
                @if($receiver)
                    <div class="p-4 bg-white shadow-md rounded w-full sm:w-3/4">
                        ...
                        <div wire:poll.30s="refreshMessages" id="messages-container"
                             class="border p-3 overflow-y-auto h-80 sm:max-h-[500px]">
                            @foreach($messages->groupBy(function($msg) {
                                ...
                                @foreach ($msgs as $msg)
                                    <div
                                        class="mb-2 group chat-text {{ $msg->sender_id === auth()->id() ? 'text-right' : 'text-left' }}">
                                        <span
                                            title="{{ $msg->created_at->setTimezone(config('app.timezone'))->format('h:i A T') }}"
                        ...
                        <div class="mt-4">
                            <form wire:submit.prevent="sendMessage">
                                <input type="text" wire:model="messageContent" class="w-full px-3 py-2 border rounded"
                                       placeholder="Type a message...">
                                <div class="flex justify-between items-center">
                                    <button type="submit" class="mt-2 px-4 py-2 bg-primary text-white rounded">Send
                                    </button>

                                    <button wire:click="refreshMessages"
                                            class="flex gap-1 items-center px-2 text-xs rounded bg-[#F56565] text-white hover:bg-[#FF7070]">
                                        <svg data-v-c9f5a6e8="" xmlns="http://www.w3.org/2000/svg" width="24"
...
<script>
    ...
    // Make sure this runs both on an initial load and after updates
