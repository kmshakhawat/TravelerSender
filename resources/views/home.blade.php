                    ...
                    <form method="GET" action="{{ route('trip.search') }}" class="w-full">
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-10">
                            <div class="relative w-full">
                                ...
                                <input wire:model="from" type="text" id="autocomplete-location" class="block w-full ps-10 form-input !py-4" name="from" />
                            </div>
                            <div class="relative w-full">
                                <span class="absolute top-[8px] text-gray-300 ps-10 text-sm">To</span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-6 opacity-50 text-primary absolute left-3 top-1/2 transform -translate-y-1/2"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>
                                <input wire:model="to" type="text" id="autocomplete-location" class="block w-full ps-10 form-input !py-4" name="to" />
                            </div>
                            <div class="relative w-full">
...
{{--    <x-faq />--}}
    ...
    <!-- Algolia InstantSearch & Autocomplete dependencies -->
    <script src="https://cdn.jsdelivr.net/npm/algoliasearch@4/dist/algoliasearch-lite.umd.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/@algolia/autocomplete-js@1.10.5/dist/autocomplete.umd.js"></script>

    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@algolia/autocomplete-theme-classic@1.8.3" />


    <script>
        const searchClient = algoliasearch('1OAJBF46K2', '66dd053667d06c7f205701f7b48e3ec9');
        document.addEventListener('DOMContentLoaded', function () {
            autocomplete({
                container: '#autocomplete-location',
                    ...
                    return [
                        {
                            sourceId: 'cities',
                            getItems() {
                                return searchClient.initIndex('cities').search(query);
                            },
                            templates: {
                                header() {
                                    return '<div class="aa-SourceHeader">Cities</div>';
                                },
                                item({item}) {
                            ...
                            onSelect({item}) {
                                document.getElementById('autocomplete-location').value = item.name;
                            }
                        },
                        {
                            sourceId: 'states',
                            getItems() {
                                return searchClient.initIndex('states').search(query);
                            },
                            templates: {
                                header() {
                                    return '<div class="aa-SourceHeader">States</div>';
                                },
                                item({item}) {
                                    return `<div>${item.name}</div>`;
                                }
                        ...
                        },
                        {
                            sourceId: 'countries',
                            getItems() {
                                return searchClient.initIndex('countries').search(query);
                            },
                            templates: {
                                header() {
                                    return '<div class="aa-SourceHeader">Countries</div>';
                                },
                                item({item}) {
                                    return `<div>${item.name}</div>`;
                                }
